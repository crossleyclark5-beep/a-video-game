...
extends RefCounted
## Authored mountain set pieces + cliff / cave overlays on the heightfield foundation.
## Hills use real elevation from GrasslandHeightField; climb routes remain switchback trails.
...
static func build(root: Node3D, result: Dictionary) -> void:
	...
	_build_cliff_bands(terrain)
	_build_hidden_caves(terrain, result)
	_build_valleys(terrain)
	_build_landing_flats(terrain)


static func _build_rolling_hills(parent: Node3D) -> void:
	## Walkable crest markers on heightfield highs — soft sculptural read without fake flats.
	var hills := [
		...
		{"pos": Vector3(-1100, 0, -200), "h": 2.5, "r": 15.0},
		{"pos": Vector3(1900, 0, -600), "h": 2.2, "r": 13.0},
		{"pos": Vector3(700, 0, 2400), "h": 2.0, "r": 12.0},
		{"pos": Vector3(-400, 0, 1400), "h": 1.9, "r": 11.0},
	]
	for i in hills.size():
		var base: Vector3 = GrasslandHeightField.snap(hills[i]["pos"])
		_hill(parent, base, float(hills[i]["h"]), float(hills[i]["r"]), i)
...
static func _build_valleys(parent: Node3D) -> void:
	var valleys := [
		[Vector3(80, 0, 90), Vector3(40, 0.04, 10)],
		[Vector3(500, 0, -200), Vector3(60, 0.04, 12)],
		[Vector3(1700, 0, 2000), Vector3(50, 0.04, 14)],
		[Vector3(-400, 0, 500), Vector3(45, 0.04, 11)],
		[Vector3(1200, 0, -500), Vector3(55, 0.04, 12)],
	]
	for i in valleys.size():
		var v: Array = valleys[i]
		var pos: Vector3 = GrasslandHeightField.snap_y(v[0], 0.03)
		StylizedMesh.add_box(parent, v[1], WorldPalette.DIRT.darkened(0.05), pos, "Valley_%d" % i, false, 1.0, &"dirt")


static func _build_cliff_bands(parent: Node3D) -> void:
	## Sheer rock faces that read from the air and block lazy climbs.
	var cliffs := [
		{"pos": Vector3(-280, 0, -200), "size": Vector3(40, 10, 6), "yaw": 25.0},
		{"pos": Vector3(1600, 0, 900), "size": Vector3(50, 8, 5), "yaw": -15.0},
		{"pos": Vector3(400, 0, -1800), "size": Vector3(35, 12, 5), "yaw": 10.0},
		{"pos": Vector3(2800, 0, 3200), "size": Vector3(55, 14, 6), "yaw": -40.0},
		{"pos": Vector3(-1600, 0, 800), "size": Vector3(42, 9, 5), "yaw": 55.0},
	]
	var root := Node3D.new()
	root.name = "CliffBands"
	parent.add_child(root)
	for i in cliffs.size():
		var c: Dictionary = cliffs[i]
		var base: Vector3 = GrasslandHeightField.snap(c["pos"])
		var node := Node3D.new()
		node.name = "Cliff_%d" % i
		node.position = base
		node.rotation_degrees.y = float(c["yaw"])
		root.add_child(node)
		var sz: Vector3 = c["size"]
		StylizedMesh.add_box(node, sz, WorldPalette.ROCK.darkened(0.06), Vector3(0, sz.y * 0.45, 0), "Face", true, 1.0, &"dirt")
		StylizedMesh.add_box(node, sz * Vector3(0.7, 0.25, 0.8), WorldPalette.ROCK.lightened(0.05), Vector3(0, sz.y * 0.85, -1.0), "Lip", false, 1.0, &"dirt")


static func _build_hidden_caves(parent: Node3D, result: Dictionary) -> void:
	## Extra 3D cave mouths — minor discoveries (map entries live in RegionDiscoveryBuilder).
	var caves := [
		{"pos": Vector3(900, 0, -200), "yaw": 40.0, "id": &"moss_cave"},
		{"pos": Vector3(-500, 0, 900), "yaw": -30.0, "id": &"echo_cave"},
		{"pos": Vector3(2200, 0, 1600), "yaw": 110.0, "id": &"dust_cave"},
	]
	var root := Node3D.new()
	root.name = "HiddenCaves"
	parent.add_child(root)
	for i in caves.size():
		var spec: Dictionary = caves[i]
		var cave := Node3D.new()
		cave.name = "Cave_%s" % String(spec["id"])
		cave.position = GrasslandHeightField.snap(spec["pos"])
		cave.rotation_degrees.y = float(spec["yaw"])
		root.add_child(cave)
		StylizedMesh.add_box(cave, Vector3(7, 4.2, 6), WorldPalette.ROCK, Vector3(0, 1.8, 0), "Hill", true, 1.0, &"dirt")
		StylizedMesh.add_box(cave, Vector3(2.2, 2.0, 2.2), Color(0.07, 0.07, 0.09), Vector3(0, 1.0, 2.4), "Mouth")
		StylizedMesh.add_box(cave, Vector3(4.5, 0.2, 4.0), WorldPalette.DIRT.darkened(0.1), Vector3(0, 0.05, 0.5), "Floor", true, 1.0, &"dirt")
		## Interior volume so cave exists in 3D, not a painted hole.
		StylizedMesh.add_box(cave, Vector3(3.5, 2.4, 3.5), WorldPalette.ROCK.darkened(0.2), Vector3(0, 1.3, -0.8), "Chamber", false, 1.0, &"dirt")
		if i == 0:
			result[&"chests"].append(
				RegionPropKit.build_chest(cave, "MossCaveChest", Vector3(0, 0.2, -0.5), ChestInteractable.Rarity.NORMAL, 36.0, "Search the moss cave")
			)


static func _build_landing_flats(parent: Node3D) -> void:
	## Open meadow pads between hubs — future aircraft landing zones (still walkable ground).
	var pads := [
		Vector3(800, 0, 400),
		Vector3(-200, 0, -800),
		Vector3(1600, 0, 2000),
	...
	var root := Node3D.new()
	root.name = "LandingFlats"
	parent.add_child(root)
	for i in pads.size():
		var p: Vector3 = GrasslandHeightField.snap_y(pads[i], 0.02)
		StylizedMesh.add_box(root, Vector3(36, 0.05, 28), WorldPalette.GRASS_LIGHT, p, "Flat_%d" % i, false, 1.0, &"grass")
		## Corner stones so the pad reads from altitude.
		for j in 4:
			var ang := float(j) * TAU * 0.25 + 0.4
			var cp := p + Vector3(cos(ang) * 16.0, 0.15, sin(ang) * 12.0)
			StylizedMesh.add_box(root, Vector3(0.6, 0.35, 0.5), WorldPalette.ROCK, cp, "FlatMark_%d_%d" % [i, j], false, 1.0, &"dirt")
