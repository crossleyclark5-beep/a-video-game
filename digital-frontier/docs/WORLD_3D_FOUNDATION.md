...
| `WorldDressingBuilder` | Biome dressing, micro-stories, trails, viewpoints (off-map) |

Build order (see `GrasslandRegionBuilder`): heightfield → hubs → corridors → terrain overlays → vegetation → **dressing** → discoveries.
...
| Dressing | **No** | Micro-stories, natural landmarks, trails, viewpoints |
